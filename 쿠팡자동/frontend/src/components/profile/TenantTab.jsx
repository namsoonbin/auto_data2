...
function TenantTab({ onSuccess, onError }) {
        ...
        <CardContent className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {/* 상태 */}
              <div className={`${tenant?.is_active ? 'bg-green-50 border-green-100' : 'bg-red-50 border-red-100'} border rounded-lg p-4 flex items-center gap-4`}>
                <div className={`${tenant?.is_active ? 'bg-green-100' : 'bg-red-100'} rounded-full p-3`}>
                  <Activity className={`size-5 ${tenant?.is_active ? 'text-green-600' : 'text-red-600'}`} />
                </div>
                <div>
                  <p className="text-gray-500 text-sm">상태</p>
                  <p className="text-black font-medium">{tenant?.is_active ? '활성' : '비활성'}</p>
                </div>
              </div>
      ...
      <Card className="shadow-lg">
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className={`bg-gradient-to-br ${tenant?.is_active ? 'from-green-500 to-green-600' : 'from-red-500 to-red-600'} rounded-lg p-6 text-center text-white`}>
              <Activity className="size-8 mx-auto mb-3 opacity-90" />
              <p className="text-2xl font-bold mb-1">{tenant?.is_active ? '활성' : '비활성'}</p>
              <p className={`${tenant?.is_active ? 'text-green-100' : 'text-red-100'} text-sm`}>계정 상태</p>
            </div>
          </div>
        </CardContent>
      </Card>
